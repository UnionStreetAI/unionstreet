...
async function* streamModelStub(opts: StreamModelOptions): AsyncGenerator<CodexEvent> {
  ...
  const pluginEchoTool = opts.tools?.find((tool) => /plugin_echo/i.test(tool.name));
  if (/use\s+(?:the\s+)?plugin\s+echo\s+tool/i.test(lastUser) && !hasToolResult && pluginEchoTool) {
    yield {
      type: "tool-call",
      call: {
        id: "stub_tool_plugin_echo",
        name: pluginEchoTool.name,
        arguments: JSON.stringify({ message: lastUser }),
      },
    };
    yield { type: "finish", reason: "tool_calls", usage: stubUsage() };
    return;
  ...
  if (/confirm\s+plugin\s+skill\s+loaded/i.test(lastUser)) {
    const loaded = /PLUGIN_SKILL[\s\S]*behavioral-plugin-skill/.test(opts.system ?? "");
    const text = loaded
      ? `plugin skill loaded for ${opts.profile}`
      : `plugin skill missing for ${opts.profile}`;
...
function buildOpenAICompatBody(opts: StreamOpenAICompatOptions): Record<string, unknown> {
  return {
    ...
    messages: convertOpenAICompatMessages(opts.messages, opts.system, shouldUsePlainToolResultMessages(opts)),
...
function convertOpenAICompatMessages(
  ...
  for (const m of messages) {
    ...
    if (m.role === "user") {
      ...
      if (plainToolResults && m.tool_calls?.length) {
        out.push({
          role: "assistant",
          content: [
            m.content,
            `Requested tool calls: ${m.tool_calls.map((call) => `${call.name}(${call.arguments || "{}"})`).join("; ")}`,
          ].filter(Boolean).join("\n"),
        });
      } else {
        out.push({
      ...
      if (plainToolResults) {
        out.push({ role: "user", content: `Tool result${m.tool_call_id ? ` for ${m.tool_call_id}` : ""}:\n${m.content}` });
      } else {
        out.push({
          role: "tool",
...
function shouldUsePlainToolResultMessages(opts: StreamOpenAICompatOptions): boolean {
  return process.env.US_OPENAI_COMPAT_TOOL_RESULTS_AS_USER === "1" || /thurgood\.cloud/i.test(opts.baseUrl);
}
