...
import { resolvePluginCapabilitiesForAgent, type ResolvedAgentPluginCapabilities } from "./plugins.ts";
...
export async function runAgentPrompt(options: AgentPromptOptions): Promise<AgentPromptResult> {
  ...
  const pluginCapabilities = await readPluginCapabilities(profile, cwd);
  const systemPrompt = await composeSystemPrompt(profile, modelId, providerId, pluginCapabilities);
  ...
  const tools = [...STARTER_TOOLS, ...(pluginCapabilities?.tools ?? []), ...(await resolveMcpToolsForAgent(profile, cwd))];
...
function parseToolArguments(raw: string): Record<string, unknown> {
  try {
    ...
    if (!first) throw new Error("JSON Parse error: Unable to parse JSON string");
...
async function composeSystemPrompt(profileName: string, modelId: string, provider: string, pluginCapabilities: ResolvedAgentPluginCapabilities | undefined): Promise<string> {
  ...
  const pluginSection = composePluginSection(pluginCapabilities);
  if (pluginSection) sections.push(pluginSection);
  return sections.join("\n\n");
}

async function readPluginCapabilities(profileName: string, cwd: string): Promise<ResolvedAgentPluginCapabilities | undefined> {
  try {
    return await resolvePluginCapabilitiesForAgent(profileName, cwd);
  } catch {
    return undefined;
  }
}

function composePluginSection(resolved: ResolvedAgentPluginCapabilities | undefined): string {
  if (!resolved || (!resolved.plugins.length && !resolved.missing.length)) return "";
  const lines = [
    "Plugins granted to this agent. Use only these plugin workflows when a task asks for plugin-specific behavior.",
  ];
  for (const plugin of resolved.plugins) {
    const manifest = plugin.manifest;
    lines.push(`- ${manifest.name}@${manifest.version}: ${manifest.description}`);
    if (manifest.capabilities.skills.length) lines.push(`  skills: ${manifest.capabilities.skills.join(", ")}`);
    if (manifest.capabilities.commands.length) lines.push(`  cli: ${manifest.capabilities.commands.join(", ")}`);
    if (manifest.capabilities.tools.length) lines.push(`  tools: ${manifest.capabilities.tools.join(", ")}`);
  }
  if (resolved.commands.length) lines.push(`CLI available through normal shell tool: ${resolved.commands.map((item) => item.command).join(", ")}`);
  if (resolved.tools.length) lines.push(`Custom model-callable tools: ${resolved.tools.map((tool) => tool.definition.name).join(", ")}`);
  for (const skill of resolved.skills) {
    lines.push(`<PLUGIN_SKILL name="${skill.name}" plugin="${skill.plugin}" path="${skill.path}">`);
    if (skill.description) lines.push(`description: ${skill.description}`);
    lines.push(skill.content);
    lines.push("</PLUGIN_SKILL>");
  }
  for (const missing of resolved.missing) lines.push(`- missing plugin: ${missing}`);
  return `<PLUGINS>\n${lines.join("\n")}\n</PLUGINS>`;
}
