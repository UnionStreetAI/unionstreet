...
const grepTool: UsTool = {
  ...
  async execute(args, ctx) {
    ...
    const proc = spawn(cmd, { stdout: "pipe", stderr: "pipe", env: toolEnvironment() });
    const [stdout, stderr] = await Promise.all([
      new Response(proc.stdout).text(),
      new Response(proc.stderr).text(),
    ]);
...
function resolveUnder(cwd: string, p: string): string {
  ...
  const resolved = isAbsolute(p) ? resolvePath(p) : resolvePath(root, p);
...
function toolEnvironment(): Record<string, string> {
  ...
  for (const name of (process.env.US_TOOL_ENV_ALLOWLIST ?? "").split(",")) {
    const trimmed = name.trim();
    if (/^[A-Za-z_][A-Za-z0-9_]*$/.test(trimmed)) allowed.add(trimmed);
  }
...
async function commandExists(cmd: string): Promise<boolean> {
  const proc = spawn(["which", cmd], { stdout: "pipe", stderr: "pipe", env: toolEnvironment() });
  await proc.exited;
