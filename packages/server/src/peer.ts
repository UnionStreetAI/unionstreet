...
import { invokeAgentRuntime } from "./runtime-invoker.ts";
...
export async function peerCall(opts: PeerCallOptions): Promise<PeerCallResult> {
  ...
  const routed = await invokeAgentRuntime({
    kind: "peer_wake",
    ...
    ...(opts.textVerbosity ? { textVerbosity: opts.textVerbosity } : {}),
  ...
  if (process.env.US_AGENTIC_PEER_WAKE === "1" && process.env.US_PEER_CALL_STUB !== "1") {
    return runAgenticPeerWake({
      targetPeer: opts.targetPeer,
      callingPeer: opts.callingPeer,
      provider,
      modelId,
...
async function runAgenticPeerWake(input: {
  targetPeer: string;
  callingPeer: string;
  provider: string;
  modelId: string;
  ...
  const { runAgentPrompt } = await import("./prompt-runner.ts");
  const sessionId = `agentic-${input.wakeKind}-${safeId(input.thread.id)}-${hashId(input.trace)}`;
  ...
  try {
    const result = await runAgentPrompt({
      profile: input.targetPeer,
      sessionId,
      trace: input.trace,
      model: { provider: input.provider, id: input.modelId },
      maxSteps: readAgenticMaxSteps(),
      prompt: [
        `Lash trace: ${input.trace}`,
        `Lash thread: ${input.thread.id} turn ${input.thread.turn}`,
        `Caller: @${input.callingPeer}`,
        `Wake kind: ${input.wakeKind}`,
        "",
        "You are being woken as a full Union Street agent loop. You may use your normal tools, including delegate and report, subject to org visibility.",
        "Decide whether to delegate to direct reports based on the task. Do not fan out blindly.",
        "If you have enough information, answer directly. If material truth must flow upward, use report.",
        "",
        `Message from @${input.callingPeer}:`,
        input.message,
      ].join("\n"),
    });
    ...
    return {
      ok: true,
      response: text,
      envelope: lashTextResult(text, {
        from: input.targetPeer,
        trace: input.trace,
        thread: input.thread,
      }),
      trace: input.trace,
    ...
    return { ok: false, error: (error as Error).message, trace: input.trace, thread: input.thread, chain: input.chain };
  }
}

function readAgenticMaxSteps(): number {
  const value = Number(process.env.US_AGENTIC_PEER_MAX_STEPS);
