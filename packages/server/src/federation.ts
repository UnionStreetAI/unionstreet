...
let federationKeysPromise: Promise<FederationKeys> | undefined;
...
async function ensureFederationKeys(): Promise<FederationKeys> {
  federationKeysPromise ??= loadOrCreateFederationKeys().catch((error) => {
    federationKeysPromise = undefined;
    throw error;
  });
  return federationKeysPromise;
}

async function loadOrCreateFederationKeys(): Promise<FederationKeys> {
  try {
  ...
  try {
    await fs.writeFile(FEDERATION_KEYS_PATH, JSON.stringify(keys, null, 2), { mode: 0o600, flag: "wx" });
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "EEXIST") throw error;
    const raw = JSON.parse(await fs.readFile(FEDERATION_KEYS_PATH, "utf8")) as FederationKeys;
    if (raw.kid && raw.publicJwk && raw.privateJwk) return raw;
    throw new Error("existing federation key file is invalid");
  }
  return keys;
}
...
function demoOrg(): FederationOrgNode[] {
  const nodes: FederationOrgNode[] = [
    ...
    node("ic-ops-observability", "Marlow Green", "Ops Observability Agent", "mgr-ops-sre", ["operator"], ["operations"]),
    node("ic-ops-incident", "Sage Robinson", "Ops Incident Agent", "mgr-ops-sre", ["operator"], ["operations"]),
    node("ic-ops-release", "Harper Young", "Ops Release Agent", "mgr-ops-sre", ["operator"], ["operations"]),
    node("ic-ops-test-automation", "Kendall Scott", "Ops Test Automation Agent", "mgr-ops-qa", ["agent"], ["operations"]),
    node("ic-ops-compliance", "Ari Collins", "Ops Compliance Agent", "mgr-ops-qa", ["agent"], ["operations"]),
    node("ic-ops-runbooks", "Shawn Reed", "Ops Runbooks Agent", "mgr-ops-qa", ["agent"], ["operations"]),
    node("ic-eng-frontend", "Dakota Bell", "Frontend Engineering Agent", "mgr-eng-apps", ["reviewer"], ["engineering"]),
    node("ic-eng-backend", "Lennox Cook", "Backend Engineering Agent", "mgr-eng-apps", ["reviewer"], ["engineering"]),
    node("ic-eng-api", "Marin Bailey", "API Engineering Agent", "mgr-eng-apps", ["reviewer"], ["engineering"]),
    node("ic-eng-runtime", "Robin Cooper", "Runtime Engineering Agent", "mgr-eng-platform", ["operator"], ["engineering"]),
    node("ic-eng-infra", "Ellis Howard", "Infrastructure Engineering Agent", "mgr-eng-platform", ["operator"], ["engineering"]),
    node("ic-eng-security", "Micah Ward", "Security Engineering Agent", "mgr-eng-platform", ["operator"], ["engineering"]),
    node("ic-gtm-strategic", "Alden Cox", "Strategic Accounts Agent", "mgr-gtm-enterprise", ["agent"], ["go-to-market"]),
    node("ic-gtm-expansion", "Shiloh Gray", "Expansion Agent", "mgr-gtm-enterprise", ["agent"], ["go-to-market"]),
    node("ic-gtm-solutions", "Remy Torres", "Solutions Agent", "mgr-gtm-enterprise", ["agent"], ["go-to-market"]),
    node("ic-gtm-demand", "Lane Peterson", "Demand Generation Agent", "mgr-gtm-content", ["agent"], ["go-to-market"]),
    node("ic-gtm-brand", "Monroe Flores", "Brand Agent", "mgr-gtm-content", ["agent"], ["go-to-market"]),
    node("ic-gtm-lifecycle", "Rowan Simmons", "Lifecycle Agent", "mgr-gtm-content", ["agent"], ["go-to-market"]),
    node("ic-finance-invoicing", "Devon Foster", "Invoicing Agent", "mgr-finance-billing", ["agent"], ["finance"]),
    node("ic-finance-revrec", "Sawyer Bryant", "Revenue Recognition Agent", "mgr-finance-billing", ["agent"], ["finance"]),
  ];
  return nodes;
}
