...
describe("peer calls", () => {
  ...
  test("peerCall_WhenTargetAgentIsConfiguredForDocker_RoutesWakeThroughRuntimeProvider", async () => {
    process.env.US_PEER_RUNTIME_STUB = "1";
    const pack = await core.readAgentPack("vp-ops");
    await core.writeAgentPack("vp-ops", {
      ...pack,
      runtime: {
        ...pack.runtime,
    ...
    const result = await core.peerCall({
      callingPeer: "coo",
      targetPeer: "vp-ops",
      message: "Wake in your sandbox.",
      trace,
      wakeKind: "delegate",
    });

    expect(result.ok, "Docker-backed peers should still appear as normal successful Lash peers to the caller.").toBe(true);
    expect(result.response, "The runtime router should wake the target through Docker instead of the head process.").toContain("routed via docker runtime");
    expect(result.thread?.id, "Runtime routing must preserve Lash thread identity for the target peer.").toBe(`vp-ops/${trace}`);
    const events = await core.queryEvents({ actor: "vp-ops", type: "runtime.workspace.ensure", trace });
    expect(events[0]?.payload, "Runtime-routed peer wakes should leave an auditable provider boundary.").toMatchObject({
      provider: "docker",
      pluginId: "runtime-docker",
      routed: true,
    });
    delete process.env.US_PEER_RUNTIME_STUB;
  });

  test("peerCall_WhenTargetAgentUsesRemoteProviderWithoutAdapter_FailsExplicitlyBeforeLocalExecution", async () => {
    const pack = await core.readAgentPack("vp-gtm");
    await core.writeAgentPack("vp-gtm", {
      ...pack,
      runtime: {
        ...pack.runtime,
    ...
    const result = await core.peerCall({
      callingPeer: "coo",
      targetPeer: "vp-gtm",
      message: "Wake remotely.",
      wakeKind: "delegate",
    });

    expect(result.ok, "Remote providers without a mechanical wake adapter must not silently run on the head node.").toBe(false);
    expect(result.error, "Unsupported remote runtime should name the missing provider adapter.").toContain("runtime invocation is not implemented for runtime-vercel yet");
  });
});
