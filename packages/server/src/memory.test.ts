...
const { FileMemoryStore } = await import("./memory.ts");
...
describe("memory store", () => {
  ...
  test("resolveMemorySyncConfig_WhenEnvOverridesAreProvided_UsesEnvUrlAndWorkspace", async () => {
    ...
    expect(cfg.workspaceId, "Profile-scoped Honcho memory should prefer the agent pack peer profile over the default local runtime workspace.").toBe("memory-sync");
  });
});
