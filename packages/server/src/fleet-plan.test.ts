...
import type { FleetPlan } from "./fleet-plan.ts";

const usHome = await mkdtemp(join(tmpdir(), "union-street-fleet-plan-test-"));
...
const {
  ...
  serializeFleetPlan,
  validateFleetPlan,
} = await import("./fleet-plan.ts");
const { readAgentPack } = await import("./agent-pack.ts");
const { readFederationConfig } = await import("./federation.ts");
const { FEDERATION_PATH, profilePaths } = await import("./paths.ts");
...
describe("fleet plans", () => {
  ...
  test("buildFleetAgentPacks_WhenPlanRequestsPlugins_GrantsPluginBundlesPerAgent", async () => {
    const plan = sampleFleetPlan("fleet_plugins", [
      { id: "fleet_plugins_root", plugins: ["github"] },
      { id: "fleet_plugins_eng", manager: "fleet_plugins_root", plugins: ["github", "vercel"] },
    ]);

    const validation = await validateFleetPlan(plan);
    const packs = buildFleetAgentPacks(plan);

    expect(validation.summary.plugins, "Fleet validation should summarize requested plugin bundles for onboarding review.").toEqual(["github", "vercel"]);
    expect(
      packs.get("fleet_plugins_eng")?.toolkit.plugins,
      "Fleet materialization must put requested plugin bundles into the agent pack so skills/tools are scoped per agent.",
