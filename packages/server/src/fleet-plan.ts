...
export function createFleetPlanningPrompt(profile: string, userPrompt: string): string {
  return [
    ...
    "fallback, mcp, cli, plugins, permissions, secrets, runtime, pulse, schedule, memory.",
    "",
    "Policy constraints:",
...
export async function validateFleetPlan(plan: FleetPlan, options: { allowExisting?: boolean } = {}): Promise<FleetPlanValidation> {
  ...
  const plugins = unique(normalized.agents.flatMap((agent) => agent.plugins ?? []));
  return {
    ok: errors.length === 0,
    errors,
    warnings,
  ...
  for (const node of org) {
    ...
    packs.set(node.id, {
      ...
      toolkit: {
        ...
        plugins: agent.plugins ?? pack.toolkit.plugins,
...
function normalizeFleetAgent(value: unknown): FleetPlanAgent {
  ...
  return {
    ...
    ...(readStringArray(value.plugins).length ? { plugins: readStringArray(value.plugins) } : {}),
