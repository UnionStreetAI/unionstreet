 ...
 * Modifications: removed pi-ai internal type imports, removed `modifyModels`
 * (handled at the AI SDK provider layer instead), inlined the Copilot model
 * list previously read from pi's model registry.
 */

import type { OAuthCredentials, OAuthLoginCallbacks, OAuthProviderInterface } from "./types.js";

// Known GitHub Copilot model IDs that may require post-login enabling.
// Inlined from pi-mono's model registry to keep us-auth self-contained.
// TODO: when US has a model registry, source this list from there.
...
async function enableAllGitHubCopilotModels(
	...
	await Promise.all(
		COPILOT_MODEL_IDS.map(async (modelId) => {
