 ...
 * subprocess managed by us-runtime), `chat.tsx` swaps the implementation
...
export async function resolveMemorySyncConfig(peer?: string): Promise<MemorySyncConfig> {
  ...
  const workspaceId = readString(rawSync.workspaceId) ?? honcho?.workspaceId ?? pack?.memory.peerProfile ?? "local";
