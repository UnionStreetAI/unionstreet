...
export interface RuntimeHeadConfig {
  ...
  provider: "local" | "kubernetes" | "aws" | "gcp" | "azure" | "vercel";
  honcho?: {
    baseUrl?: string;
    workspaceId?: string;
    apiKeyEnv?: string;
...
export interface RuntimeStorageConfig {
  provider: "local" | "volume" | "s3" | "gcs" | "azure-blob" | "vercel-blob" | "modal-volume" | "daytona-volume";
  bucket?: string;
  volume?: string;
  mountPath: string;
  persistent: boolean;
...
export interface RuntimeIngressConfig {
  provider: "local" | "http" | "vercel" | "aws-alb" | "gcp-lb" | "azure-app-gateway" | "kubernetes-ingress" | "modal" | "daytona";
  url?: string;
  internalUrl?: string;
  public: boolean;
...
export async function resolveAgentRuntime(profile: string): Promise<ResolvedAgentRuntime> {
  ...
  const runtime = normalizeAgentRuntime(mergeRecords(DEFAULT_RUNTIME as unknown as Record<string, unknown>, globalRuntime, profileRuntime));
...
function normalizeAgentRuntime(value: Record<string, unknown>): AgentRuntimeConfig {
  ...
  return {
    ...
    head: {
      ...
      provider: readEnum(headRaw.provider, ["local", "kubernetes", "aws", "gcp", "azure", "vercel"]) ?? DEFAULT_RUNTIME.head.provider,
    ...
    storage: {
      provider: readEnum(storageRaw.provider, ["local", "volume", "s3", "gcs", "azure-blob", "vercel-blob", "modal-volume", "daytona-volume"]) ?? storageProviderFor(provider),
      ...
      mountPath: readString(storageRaw.mountPath) ?? readString(workspaceRaw.root) ?? DEFAULT_RUNTIME.storage.mountPath,
    ...
    ingress: {
      provider: readEnum(ingressRaw.provider, ["local", "http", "vercel", "aws-alb", "gcp-lb", "azure-app-gateway", "kubernetes-ingress", "modal", "daytona"]) ?? ingressProviderFor(provider),
...
function readProvider(value: unknown): RuntimeProvider | undefined {
  return readEnum(value, ["local", "docker", "kubernetes", "aws", "gcp", "azure", "vercel", "modal", "daytona"]);
}
